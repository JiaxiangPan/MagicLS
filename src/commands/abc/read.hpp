...
namespace alice
{
    ...
    {
    public:
        explicit read_command(const environment::ptr &env) : command(env, "replaces the current network by the network read from file by ABC parser")
        {
    ...
    protected:
        void execute() override
        {
            ...
            pabc::Abc_Ntk_t *pNtk = pabc::Io_Read((char *)(file_name.c_str()), pabc::Io_ReadFileType((char *)(file_name.c_str())), fCheck, fBarBufs);

            if (pNtk == NULL)
                std::cerr << "Error: Empty input format.\n";
